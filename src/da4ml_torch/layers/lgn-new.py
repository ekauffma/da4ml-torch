...
class ReplayLogicConv2d(ReplayBase):
    ...
    def call(self, inputs: FixedVariableArray):
        ...
        conn_0: np.ndarray = self.module.connections.indices[0].cpu().detach().numpy()  # type: ignore
        # lut_0: np.ndarray = self.module.lut_ids_0.cpu().detach().numpy()  # type: ignore
        lut_0: np.ndarray = self.module.get_luts_and_ids()[0][0]  # type: ignore
        ...
        for level in range(1, self.module.tree_depth + 1):
            ...
            lut_level = self.module.get_luts_and_ids()[0][level]  # type: ignore
            # conn_level = getattr(self.module, f'connection_indices_{level}')
            conn_level = self.module.connections.indices[level]
...
class ReplayLogicDense(ReplayBase):
    ...
    def call(self, inputs: FixedVariableArray):
        ...
        _, lut_ids = self.module.get_luts_and_ids()  # type: ignore
